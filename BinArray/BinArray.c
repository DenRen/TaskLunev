...
int baRetain (struct BinArray* arr, int num_bits) {
    ...
    if ((arr->buf_ = (char*) calloc (sizeof (uint8_t),  num_bytes)) == NULL) {
        return -1;
    }
...
int baResize   (struct BinArray* arr, int new_num_bits) {
   ...
   CHECK_PBA (arr);
    ...
    char* new_buf = (char*) realloc (arr->buf_, sizeof (uint8_t) * new_num_bytes);
    if (new_buf == NULL) {
        return -1;
    }
...
static inline bool _baGetValue (struct BinArray arr, int num_bit) {
    uint8_t obj = *(arr.buf_ + num_bit / 8);
    
    return (obj & (1U << (7 - num_bit % 8))) != 0;
}

int baGetValue (struct BinArray arr, int num_bit) {
...
// Without checking for correctness (for speed)
static inline void _baSetValue (struct BinArray arr, int num_bit, bool val) {
    char* obj = arr.buf_ + num_bit / 8;
    char mask = 1U << (7 - num_bit % 8);
...
int baFind_True (struct BinArray arr) {
    ...
    if (i == num_u64) {
        ...
        const uint8_t* buf_u8 = buf_u64 + i;
